...
rootftpdir ='/pub/org/chg/products/EWX/data/forecasts/CHIRPS-GEFS_precip/10day/precip_75prcn/' #'/pub/org/chg/products/EWX/data/forecasts/CHIRPS-GEFS_precip/daily_last/' # '/pub/org/chg/products/EWX/data/forecasts/CHIRPS-GEFS_precip/dekad_first/'
rootoutputdir =  params.dataTypes[36]['inputDataLocation']
...
def getFilesForYearAndMonth(ftp,yearToGet, monthToGet, day):
    ...
    files = ftp.nlst()
    filteredfiles = [f for f in files if 'data-75prcn_'+str(yearToGet) in f]
    for fileToProcess in filteredfiles:
        if validFile.search(fileToProcess):
            if  re.search('data-75prcn_'+str(yearToGet), fileToProcess):
                print fileToProcess
